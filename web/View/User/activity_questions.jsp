<%-- 
    Document   : activity_questions
    Created on : Mar 11, 2026, 11:51:10 AM
    Author     : ADMIN
--%>
...
<html lang="en">
    <head>
        <meta charset="UTF-8">
        <title>${uPro.username} - Questions - DevQuery</title>
                    ...
                    <div class="row mt-4">
                        ...
                        <div class="col-md-10">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <h4 style="font-weight: 400; font-size: 21px;">${totalRecords} Questions</h4>
                            </div>

                            <div class="list-group">
                                <c:forEach items="${itemsList}" var="q">
                                    <div class="list-group-item list-group-item-action d-flex p-3">
                                        <div class="text-center me-3" style="min-width: 60px;">
                                            <span class="badge ${q.score > 0 ? 'bg-success' : 'bg-secondary'} rounded-pill fs-6">${q.score}</span>
                                            <div class="text-muted" style="font-size: 11px; margin-top: 4px;">votes</div>
                                        </div>
                                        <div class="flex-grow-1">
                                            <div class="d-flex justify-content-between align-items-center mb-1">
                                                <a href="${pageContext.request.contextPath}/question?id=${q.questionId}" class="text-decoration-none fw-bold" style="color: #0074cc; font-size: 15px;">${q.title}</a>
                                                <span class="text-muted" style="font-size: 12px;"><fmt:formatDate value="${q.createdAt}" pattern="MMM dd, yyyy"/></span>
                                            </div>

                                            <div class="text-muted mb-2" style="font-size: 13px; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; word-break: break-word;">
                                                ${q.body}
                                            </div>

                                            <div>
                                                <c:forEach items="${q.tags}" var="t">
                                                    <span class="badge bg-light text-dark border me-1" style="font-weight: normal;">${t}</span>
                                                </c:forEach>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <c:if test="${empty itemsList}">
                                    <div class="text-center text-muted p-5">User has not asked any questions yet.</div>
                                </c:if>
                            </div>
                            ...
                            <c:if test="${totalPage > 1}">
                                <div class="mt-4">
                                    <c:forEach begin="1" end="${totalPage}" var="i">
                                        <a href="?id=${uPro.userId}&tab=questions&page=${i}" class="btn btn-sm ${currentPage == i ? 'btn-primary' : 'btn-outline-secondary'}">${i}</a>
                                    </c:forEach>
                                </div>
                            </c:if>
                        </div>
